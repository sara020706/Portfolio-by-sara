...
const Badges: React.FC = () => {
  ...
  return (
    <section id="badges" className="py-20 bg-gradient-to-b from-[#0d2137] via-[#0c1929] to-[#0d2137] relative border-t border-cyan-500/20">
      {/* Background Effects */}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {badges.map((badge, index) => {
            ...
            return (
              <div
                key={index}
                className={`group bg-slate-800/40 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 shadow-2xl hover:shadow-cyan-500/20 transition-all duration-500 hover:border-cyan-500/50 relative flex flex-col ${
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  <a
                    ...
                    className="flex items-center gap-1 px-2 py-1 bg-cyan-500/20 border border-cyan-500/30 rounded-md hover:bg-cyan-500/30 transition-colors duration-200 opacity-0 group-hover:opacity-100 flex-shrink-0"
                ...
                <div className="flex-1 flex flex-col min-h-0">
                  <h3 className="text-lg font-bold text-white group-hover:text-cyan-400 transition-colors duration-300 line-clamp-2 mb-3">
                    {badge.title}
                  </h3>
                  
                  <div className="flex items-center gap-2 text-gray-400 mb-2">
                    <Star size={16} className="text-cyan-400 flex-shrink-0" />
                  ...
                  <div className={`mb-3 ${isExpanded ? '' : 'flex-1 overflow-hidden'}`}>
                    ...
                    {shouldTruncate && (
                      <button
                        ...
                        className="flex items-center gap-1 text-cyan-400 text-xs font-medium mt-2 hover:text-cyan-300 transition-colors"
                      >
                        {isExpanded ? (
                          <>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-slate-700/50 mt-auto">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-cyan-400 font-medium">Earned</span>
                    </div>
                    <a
                      href={badge.verificationUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-3 py-1 bg-cyan-500/20 border border-cyan-500/30 rounded-full hover:bg-cyan-500/30 transition-colors duration-200 text-xs text-cyan-300 font-medium whitespace-nowrap"
                    >
                      View Badge
                    </a>
                  </div>
